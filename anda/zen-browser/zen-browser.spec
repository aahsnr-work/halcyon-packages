# Ported from terrapkg/packages frawhide (anda/apps/zen-browser) on 2026-09-22
# — a SOURCE build of the Firefox-derived browser (zen.source.tar.zst +
# ./mach build, clang/lld toolchain). Deviations from Terra:
#   * anda-srpm-macros / terra-appstream-helper are Terra build-env packages —
#     removed; %desktop_file_install / %terra_appstream / %desktop_file_validate
#     replaced with the plain Fedora equivalents (marked inline).
#   * %_appsdir / %_hicolordir are defined below so Terra's %files lines stay
#     untouched.
#   * ExclusiveArch is x86_64 only (no aarch64 builds in this repo).
#   * Source0 is fetched by mock at SRPM-build time; the packaging files
#     (Source1..6) are repo files. The spec is otherwise Terra's, verbatim.
...
# this repo's launcher/desktop-file layout macros (Terra build-env names)
...
# Strip internal updater - updates flow through the package manager.
...
# Desktop entry — upstream: %desktop_file_install %{S:2}
%{__install} -Dpm644 %{SOURCE2} %{buildroot}%{_appsdir}/%{appid}.desktop
...
# upstream: %terra_appstream -o %{S:6}
%{__install} -Dpm644 %{SOURCE6} %{buildroot}%{_metainfodir}/%{appid}.metainfo.xml
