Name:          xwiimote-ng
Summary:       An open-source device driver for Nintendo Wii / Wii U remotes.

Version:       3.0.1
Release:       1%{?dist}
%define debug_package %{nil}

License:       MIT
URL:           https://github.com/dev-0x7C6/xwiimote-ng
Source:        %{url}/archive/refs/tags/v%{version}.tar.gz

ExclusiveArch: x86_64

BuildRequires: cmake
BuildRequires: gcc
BuildRequires: gcc-c++
BuildRequires: ninja-build
BuildRequires: systemd-devel
BuildRequires: systemd-rpm-macros

Requires:      systemd-udev

%description
An open-source device driver for Nintendo Wii / Wii U remotes.

%prep
%autosetup -p1 -n %{name}-%{version}

%build
%cmake
%cmake_build

%install
%cmake_install
# upstream builds no udev rules; ship the ones from res/ so the hidraw
# devices are accessible without hand-editing
install -Dpm0644 res/70-udev-xwiimote.rules \
  %{buildroot}%{_udevrulesdir}/70-xwiimote.rules

%files
%license LICENSE
%{_libdir}/lib%{name}.so.%{version}
%{_libdir}/lib%{name}.so.3
%{_libdir}/lib%{name}.so
%{_includedir}/%{name}.h
%{_datadir}/pkgconfig/lib%{name}.pc
%{_udevrulesdir}/70-xwiimote.rules

%changelog
* Wed Sep 23 2026 halcyon-autobump <aahsnr041@proton.me>
- converted to an explicit Release and changelog for the Copr build
