...
- Registry: `ci/packages.toml` — 40 packages in 3 dependency batches
...
| path                 | what                                                                                                                                                                                   |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `anda.hcl`           | root manifest — `strip_prefix = "anda/"`; anda discovers the repo recursively                                                                                                          |
...
| `andax/`             | (removed — the sweepers are self-contained)                                                                                                                                            |
...
| `deferred/`          | parked packages with a `DEFERRED.md` blocker note; not in the registry                                                                                                                 |
...
3. `buildN` runs `anda build anda/<pkg> -c halcyon-f44-x86_64` in the builder
...
| ✗ Long builds (browsers, big Rust trees) consume GitHub Actions minutes (zen-browser is the heavy one; public repos get free standard runners) |
...
5. Build locally (or push — the workflow picks it up):
   `podman run --rm -it --privileged -v "$PWD":/h -w /h ghcr.io/<owner>/halcyon-builder:f44 anda build anda/<pkg> -c halcyon-f44-x86_64`
...
- **Registered (40)** — the Hyprland stack (`hyprutils`, `hyprlang`,
  ...
  binary/tool wrappers (`bun`, `dust`, `lazygit`, `pandoc`, `starship`,
  `yazi`, `zellij`, `zotero`, `xwiimote-ng`, `ticktick`, `chafa`, `atuin`,
  `bat`, `eza`, `pixi`, `tealdeer`, `uv`, `cava`, `gnuplot`) and
  `zen-browser`. The CLI-tool set is required to install from this repo even
  when Fedora/Terra carry the same names — the repo file therefore sets
  `priority=1`. `zotero` and `chafa` are full source builds following
  terrapkg's specs; `zen-browser` follows **Terra's source-build spec** — the
  Firefox-class mach build of zen.source.tar.zst (x86_64 only, no twilight).
  `xwiimote-ng` has never been through a validated build — its first anda run
  is the validation. Every `anda/` directory is registered: no candidates, no
  deferred packages. bazaar and bazzite-portal are deliberately NOT built
  here — they install from COPR `ublue-os/packages` and the Terra repo
...
- **TeX Live**: `tools/texlive-splitter/` still generates Arch-style grouped
