...
# slow route to Copr's S3-backed downloads? the Cloudflare Worker mirror:
...
| `repo/halcyon-mirror.repo` + `repo/cloudflare-worker.js` | the Cloudflare Worker mirror: consumer drop-in + the worker source it runs |
